...
const CategoryPage = () => {
  ...
  if (error || !category) {
    return (
      <div style={{ minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
        ...
        <Link to="/" style={{ marginTop: '20px', color: 'var(--accent-main)' }}>Return to Home</Link>
      </div>
    );
  }
  ...
  return (
    <main style={{ minHeight: '100vh', padding: '40px 20px', color: 'white', maxWidth: '1400px', margin: '0 auto' }}>
      <Helmet>
        <title>{category.meta_title || `${category.name} Prompts - PromptKing`}</title>
        <meta name="description" content={category.meta_description || category.description || `Browse our collection of ${category.name} prompts.`} />
        {category.focus_keyword && <meta name="keywords" content={category.focus_keyword} />}
        <link rel="canonical" href={`https://promptking.in/category/${category.slug}`} />
        ...
        <meta property="og:url" content={`https://promptking.in/category/${category.slug}`} />
