...
/**
 * The slug for wallpapers saved without a category.
 *
 * Not a row in wallpaper_categories - it is computed from whatever has no
 * category_id, so the chip appears the moment one exists and disappears again
 * when the last of them is filed. Without it those wallpapers are reachable
 * only under "All": no chip lists them, and the counts on the chips visibly
 * fail to add up to the total, which reads as a broken filter rather than as
 * an editorial gap.
 */
