 ...
 */
/**
 * Categories, each with a wallpaper to wear as its cover.
 *
 * A category row that is a row of words is a row nobody clicks; the point of
 * putting these on the home page is that the pictures do the explaining. The
 * cover is simply the first wallpaper in the category, which is not arbitrary:
 * fetchWallpapers orders featured first, then by sort_order — so a category's
 * face is whichever wallpaper an admin has already promoted to the front of it.
 *
 * Both halves are cached, so this join costs nothing beyond the array walk.
 */
export async function fetchWallpaperCategoryCovers() {
  ...
  return categories
    .map((category) => {
      const cover = wallpapers.find((w) => w.categorySlug === category.slug);
      // fetchWallpaperCategories already drops the empty ones, so this only
      // fires if the two caches disagree mid-refresh.
      return cover ? { ...category, image: cover.image } : null;
    })
    .filter(Boolean);
}
