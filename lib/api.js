...
const getBaseUrl = () => {
  ...
  if (typeof window !== 'undefined' && 
    ...
    return ''; // Uses the same domain for API requests
  }
...
// Safely parse JSON — if the server returns HTML (e.g. 404 page), don't crash
const safeJson = async (res) => {
  ...
  try {
    ...
    console.warn(`[api] Non-JSON response from ${res.url} (${res.status}):`, text.slice(0, 120));
    return null;
  }
};
